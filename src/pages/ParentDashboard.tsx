...
import { onValue, ref as dbRef, database, set } from '@/lib/firebase';
...
const ParentDashboard = () => {
  ...
  useEffect(() => {
    const activeRef = dbRef(database, '/buses/bus_1/isActive');
    const unsub = onValue(activeRef, (snap) => {
      setIsActive(!!snap.val());
      setLoading(false);
    });
  ...
  useEffect(() => {
    ...
    const unsub = onValue(locRef, (snap) => {
      ...
      if (data && typeof data.latitude === 'number' && typeof data.longitude === 'number') {
        setLiveBusPos({ lat: data.latitude, lng: data.longitude });
      }
    });
    return () => { if (typeof unsub === 'function') unsub(); };
  }, []);
  ...
  const handleStart = () => {
    ...
    const id = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        set(dbRef(database, '/buses/bus_1/path'), (prevPath) => {
