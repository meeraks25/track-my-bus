...
const DriverDashboard = () => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "buses", "bus_1"), (docSnap) => {
      ...
      if (data && data.location) {
        setBusPos({ lat: data.location.latitude, lng: data.location.longitude });
      }
    });
    return () => unsub();
  }, []);
  ...
  const handleStart = () => {
    ...
    const id = navigator.geolocation.watchPosition(
      async (pos) => {
        ...
        // Add to path subcollection
        await addDoc(collection(db, "buses", "bus_1", "path"), {
          lat: latitude,
          lng: longitude,
          timestamp
        });
  ...
  const handleStop = () => {
    setIsTracking(false);
    setDoc(doc(db, "buses", "bus_1"), { isActive: false }, { merge: true });
    if (watchId !== null) {
